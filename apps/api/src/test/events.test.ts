...
const config: RuntimeConfig = { databasePath: join(directory, 'events.sqlite'), publicUrl: 'https://tabletop.example', port: 3000,
  store: { name: 'Test Store', location: '42 Game St, Seattle; Upstairs', timeZone: 'America/Los_Angeles' } };
let app: Awaited<ReturnType<typeof createApp>>;
const validEvent = { name: 'Friday, Magic; special \\ edition', templateId: 'magic', format: 'Commander', startsAtLocal: '2099-06-12T18:00', capacity: 2 };
before(async () => { app = await createApp(config, false); await app.init(); });
after(async () => { await app?.close(); rmSync(directory, { recursive: true, force: true }); });
const http = () => request(app.getHttpServer());
async function create(capacity = 2) { return (await http().post('/api/events').send({ ...validEvent, capacity }).expect(201)).body; }
...
test('end-to-end create → agenda → detail → registration → full; duplicate normalization', async () => {
  ...
  assert.ok(settings.templates.some((item: { name: string }) => item.name === 'Magic: The Gathering'));
  ...
  await http().post(`/api/events/${event.id}/registrations`).send({ name: '  Alex   Smith  ' }).expect(201);
  const duplicate = await http().post(`/api/events/${event.id}/registrations`).send({ name: 'ＡＬＥＸ smith' }).expect(409);
  ...
  const full = await http().post(`/api/events/${event.id}/registrations`).send({ name: 'Jordan' }).expect(409);
...
test('HTTP requests competing for the last seat produce exactly one success', async () => {
  ...
  const results = await Promise.all(Array.from({ length: 10 }, (_, i) => http().post(`/api/events/${event.id}/registrations`).send({ name: `Player ${i}` })));
...
test('independent SQLite connections competing in worker threads cannot oversell', async () => {
  ...
  try {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      `, { eval: true, workerData: { repository: require.resolve('../database/sqlite-events.repository'), config, id: event.id, name: `Worker ${i}` } });
      workers.push(worker);
...
test('validation rejects invalid capacities, names, templates, formats and unexpected fields', async () => {
  for (const capacity of [0, -1, 31, 1.5, '2', null]) await http().post('/api/events').send({ ...validEvent, capacity }).expect(400);
  await create(30);
  for (const change of [{ name: '   ' }, { templateId: 'unknown' }, { format: 'Expanded' }, { extra: true }, { startsAtLocal: '2099-02-30T12:00' }, { startsAtLocal: '2000-01-01T12:00' }]) {
    await http().post('/api/events').send({ ...validEvent, ...change }).expect(400);
  }
  const event = await create();
  for (const name of ['', '  ', 'x'.repeat(81), null, 123]) await http().post(`/api/events/${event.id}/registrations`).send({ name }).expect(400);
...
test('daylight-saving gaps and repeated local times are rejected explicitly', async () => {
  ...
  const gap = await http().post('/api/events').send({ ...validEvent, startsAtLocal: '2099-03-08T02:30' }).expect(400);
  assert.match(gap.body.message, /does not exist/);
  const repeated = await http().post('/api/events').send({ ...validEvent, startsAtLocal: '2099-11-01T01:30' }).expect(400);
...
test('calendar invite parses with an independent library and preserves title, UTC times, location and UID', async () => {
  const event = await create();
  const invite = await http().get(`/api/events/${event.id}/calendar.ics`).expect(200).expect('Content-Type', /text\/calendar/);
  assert.match(invite.headers['content-disposition'], /attachment; filename=/);
  const parsed = new ICAL.Event(new ICAL.Component(ICAL.parse(invite.text)).getFirstSubcomponent('vevent')!);
  ...
  const qr = await http().get(`/api/events/${event.id}/qr.png`).expect(200).expect('Content-Type', /image\/png/);
...
test('a fourth non-card template works without changing core logic; events survive reopening', () => {
  ...
  let id: string;
  try {
    const templates = new TemplatesService([...GAME_TEMPLATES, { id: 'chess', name: 'Chess', formats: ['Rapid'], defaultDurationMinutes: 45, defaultCapacity: 8 }]);
    const service = new EventsService(repository, templates, config);
    const event = service.create({ ...validEvent, templateId: 'chess', format: 'Rapid', capacity: 8 });
    id = event.id;
  ...
  try { assert.equal(reopened.find(id!)?.gameName, 'Chess'); } finally { reopened.onModuleDestroy(); }
});
...
test('database constraints reject direct overbooking and invalid event capacity', async () => {
  ...
  try {
    const insert = db.prepare('INSERT INTO registrations(id,event_id,name,normalized_name) VALUES (?,?,?,?)');
    insert.run(randomUUID(), event.id, 'First', 'first');
    assert.throws(() => insert.run(randomUUID(), event.id, 'Second', 'second'), /event_full/);
    assert.throws(() => db.prepare('UPDATE events SET capacity = 31 WHERE id = ?').run(event.id), /CHECK constraint/);
  } finally { db.close(); }
});
