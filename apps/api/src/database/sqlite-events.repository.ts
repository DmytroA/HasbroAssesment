...
export class SqliteEventsRepository extends EventsRepository implements OnModuleDestroy {
  ...
  list(): EventSummary[] { return this.db.prepare(`${SELECT_EVENTS} ORDER BY e.starts_at, e.id`).all() as unknown as EventSummary[]; }
  find(id: string): EventSummary | undefined { return this.db.prepare(`${SELECT_EVENTS} WHERE e.id = ?`).get(id) as unknown as EventSummary | undefined; }
  create(event: NewEvent): EventSummary {
    this.db.prepare(`INSERT INTO events(id,name,template_id,game_name,format,starts_at,ends_at,time_zone,location,capacity)
      VALUES (?,?,?,?,?,?,?,?,?,?)`).run(event.id, event.name, event.templateId, event.gameName, event.format,
      event.startsAt, event.endsAt, event.timeZone, event.location, event.capacity);
  ...
  register(eventId: string, name: string, normalizedName: string): RegistrationResult {
    ...
    try {
      ...
      else if (this.db.prepare('SELECT 1 FROM registrations WHERE event_id=? AND normalized_name=?').get(eventId, normalizedName)) {
        result = { status: 'duplicate' };
      } else if (event.registrationCount >= event.capacity) result = { status: 'full' };
      else {
        const receipt = { id: randomUUID(), eventId, name };
        this.db.prepare('INSERT INTO registrations(id,event_id,name,normalized_name) VALUES (?,?,?,?)').run(receipt.id, eventId, name, normalizedName);
      ...
      if (message.includes('UNIQUE constraint failed: registrations.event_id')) return { status: 'duplicate' };
      if (message.includes('database is locked') || message.includes('database is busy')) return { status: 'busy' };
      throw error;
    }
  }
}
