...
const event = { id: 'event-1', name: 'Friday Magic', templateId: 'magic', gameName: 'Magic: The Gathering', format: 'Commander', startsAt: '2099-06-13T01:00:00.000Z', endsAt: '2099-06-13T04:00:00.000Z', timeZone: 'America/Los_Angeles', location: '123 Main Street', capacity: 1, registrationCount: 0, registrationUrl: 'https://example.com/events/event-1/register' };
afterEach(() => { cleanup(); vi.restoreAllMocks(); });
function open() {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false }, mutations: { retry: false } } });
  return render(<QueryClientProvider client={client}><MemoryRouter initialEntries={['/events/event-1/register']}><Routes><Route path="/events/:id/register" element={<RegisterPage/>}/></Routes></MemoryRouter></QueryClientProvider>);
}
test('player registers and sees a confirmed seat and calendar download', async () => {
  vi.spyOn(api, 'event').mockResolvedValue(event);
  const register = vi.spyOn(api, 'register').mockResolvedValue({ id: 'registration-1', eventId: event.id, name: 'Alex' });
  open();
  ...
  expect(screen.getByRole('link', { name: 'Add to calendar' }).getAttribute('href')).toBe('/api/events/event-1/calendar.ics');
});
...
test('a last-seat server rejection is shown even if the UI previously showed availability', async () => {
  vi.spyOn(api, 'event').mockResolvedValue(event);
  vi.spyOn(api, 'register').mockRejectedValue(new ApiError('This event is full. No seats remain.', 409));
  open();
  await userEvent.type(await screen.findByLabelText('Your name'), 'Alex');
  await userEvent.click(screen.getByRole('button', { name: 'Confirm registration' }));
