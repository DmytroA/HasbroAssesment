...
export const eventTime = (event: EventSummary) => `${formatDate(event.startsAt, event.timeZone, { hour: 'numeric', minute: '2-digit' })} – ${formatDate(event.endsAt, event.timeZone, { hour: 'numeric', minute: '2-digit', timeZoneName: 'short' })}`;
export function groupByDay(events: EventSummary[]) {
  ...
  for (const event of [...events].sort((a, b) => a.startsAt.localeCompare(b.startsAt))) {
    const key = formatDate(event.startsAt, event.timeZone, { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' });
